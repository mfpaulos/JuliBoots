...
#load derivative table in Slava/Sheers conventions

function loadTableOLD(file::String; oddL=false, label="Vanilla N=0")
    ...
    for lct=1:nrspins
        ...
        nrpoles=convert(Int,myread(f))
        polelist=Array(BigFloat,nrpoles)
        polecoeffs=Array(BigFloat,plen,nrpoles)

        for ct=1:nrpoles
                polelist[ct]=myread(f)
        end

        for pct=1:plen
                for ct=1:nrpoles
                        polecoeffs[pct,ct]=myread(f)
                end
        end
        ...
        for i=1:plen
            polearray=[Pole(1,polelist[j],polecoeffs[i,j]) for j=1:length(polelist)]
